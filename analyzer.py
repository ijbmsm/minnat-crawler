...
## 실적이 0 인 카테고리 — 놓치고 있거나, 기준이 흐리다

342건을 세어 보니 아래가 0 건이었다 (2026-09-25). 억지로 채우지 말되,
**해당하면 반드시 쓰라.** 특히 civil_judgment 는 정치인 민사소송이 흔한데 0 이다.

### civil_judgment — 민사에서 졌을 때
형사(criminal_conviction)와 **다른 축**이다. 검찰·형벌이 아니라 법원의 민사 판단이다.
- "법원, OOO 에 손해배상 5천만원 **지급 명령**" → civil_judgment
- "명예훼손 **손해배상 청구 인용**" → civil_judgment
- "OOO 가 제기한 소송 **기각**" → 진 쪽이 행위자다. 원고가 졌으면 원고가 actor
- "검찰이 기소" → 이건 형사다. criminal_conviction

### attendance_record · politician_sns 는 전용 소스가 있어야 한다
일반 뉴스 기사에는 거의 안 나온다. **기사를 억지로 여기 넣지 마라.**
- "OOO 의원 본회의 출석률 32%" 처럼 **출석률 수치가 기사 본문에 있을 때만**
  attendance_record
- 정치인 본인 계정 글을 **그대로 옮긴 것**만 politician_sns. 언론이 인용 보도한
  것은 controversial_statement 다

### bill_promulgated · bill_enforced
"공포", "관보 게재" → bill_promulgated / "시행", "효력 발생" → bill_enforced
키워드 판정이 먼저 걸리므로 보통 자동으로 잡힌다.

## policy_record 와 controversial_statement 가리기

둘이 각각 75건·68건으로 가장 많다. 경계가 흐리면 같은 사건이 두 갈래로 나뉜다.

| 기사 | 판정 |
| --- | --- |
| 법안 발의·표결·정책 **내용**을 다룸 | policy_record |
| 정책에 대한 **말**이 기사의 중심 | controversial_statement |
| "OOO, 부동산 대책 발표" | policy_record |
| "OOO, 부동산 대책 두고 '무능' 비판" | controversial_statement |
| "OOO 의원, 반도체특별법 대표발의" | bill_proposed (입법 키워드 우선) |

## self_admission 은 본인의 공식 인정만

- "본인이 사과문 발표", "제 불찰이었다" → self_admission
- "당이 대신 유감 표명" → self_admission 아님. controversial_statement
- "의혹을 부인" → self_admission 아님
...
**여기서는 헤드라인도 요약도 쓰지 않는다.** 판정만 한다.
글은 판정을 통과한 기사에 대해서만 따로 쓴다(writer.py) — 예전에는 한 번에
다 만들었고, 그래서 버려질 기사의 헤드라인과 요약까지 생성 비용을 냈다.
